...
public class BayesianNetwork {
	...
	return L;
     ...
     /**
      * generate a random assignment always consistent with evidence
      * @param  e evidence
      * @return   the generated assignment
      */
     public Assignment randomSample(Assignment e){
       List<RandomVariable> rv = this.getVariableListTopologicallySorted();
       Assignment s = e.copy();

       for (RandomVariable r: rv){
         if (!s.containsKey(r)) {
           s.put(r, randomAssign(r, s));
         }
       }
     ...
     /**
      * test if evidence is consistent with event
      * @param  event    the event assignment
      * @param  evidence the evidence assignment
      * @return          true if consistent, false if not
      */
     public boolean consistent(Assignment event, Assignment evidence){
       for (RandomVariable r: evidence.variableSet()){
         if (!event.get(r).equals(evidence.get(r))){
           return false;
         }
       }
       return true;
     ...
     /**
      * randomly assign a value to a variable
      * @param  X the RandomVariable
      * @param  e evidence assignment
      * @return   the value in X's domain
      */
     public Object randomAssign(RandomVariable X, Assignment e) {
       double prob = 0;
       double random = java.util.concurrent.ThreadLocalRandom.current().nextDouble();
       int sizeX = X.getDomain().size();

       for (int i = 0; i < sizeX; i++) {
         Assignment a = e.copy();
         Object o = X.getDomain().get(i);
         a.put(X, o);
         prob = prob + getProb(X, a);
         if (random < prob) {
           return o;
         }
       }
       ...
       return X.getDomain().get(sizeX-1);
     }
